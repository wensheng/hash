...
PAGES_SOURCE_LOCATION = os.environ.get(
    "TLDR_PAGES_SOURCE_LOCATION", "https://raw.githubusercontent.com/tldr-pages/tldr/main/pages"
).rstrip("/")
DOWNLOAD_CACHE_LOCATION = os.environ.get(
    "TLDR_DOWNLOAD_CACHE_LOCATION", "https://github.com/tldr-pages/tldr/releases/latest/download/tldr.zip"
)
...
def load_page_from_cache(command: str, platform: str, language: str, system_cache: bool = False) -> Optional[str]:
    try:
        with get_cache_file_path(command, platform, language, system_cache).open("rb") as cache_file:
...
def store_page_to_cache(page: str, command: str, platform: str, language: str) -> Optional[str]:
    try:
...
def get_page_for_platform(
    command: str, platform: str, remote: str, language: str, only_use_cache: bool = False, system_cache: bool = False
) -> str:
    ...
    if USE_CACHE and system_cache and get_cache_file_path(command, platform, language, system_cache).is_file():
        ...
        try:
            data = urlopen(Request(page_url, headers=REQUEST_HEADERS), timeout=10, context=URLOPEN_CONTEXT).read()
...
def update_page_for_platform(command: str, platform: str, remote: str, language: str) -> None:
...
def get_language_list() -> List[str]:
    ...
    languages = list(map(get_language_code, filter(lambda x: not (x == "C" or x == "POSIX" or x == ""), languages)))
        ...
        for platform in platforms:
            for language in languages:
                ...
                try:
                    result.append((
                        get_page_for_platform(
                            command, platform, remote, language, only_use_cache=True, system_cache=True
                        ),
                        platform,
                    ))
                    break  # Don't want to look for the same page in other langs
...
DEFAULT_COLORS = {"name": "bold", "description": "", "example": "green", "command": "red", "parameter": ""}
    ...
    if get_cache_dir().exists():
        for platform in platforms:
            for language in languages:
                ...
                if not path.exists():
                    continue
                commands += [f"{file.stem}" for file in path.iterdir() if file.suffix == ".md"]
    return commands
    ...
    for line in page:
        ...
        # Handle an example command
            ...
            for item in COMMAND_SPLIT_REGEX.split(line):
                item, replaced = PARAM_REGEX.subn(lambda x: colored(x.group("param"), *colors_of("parameter")), item)
                if not replaced:
        ...
        try:
            ...
            for entry in zipfile.namelist():
                ...
                if match:
                    ...
                    cached += 1
            print(f"Updated cache for language {language}: {cached} entries")
        except Exception:
            print(f"Error: Unable to update cache for language {language} from {cache_location}")
...
def create_parser() -> ArgumentParser:
    parser = ArgumentParser(
        prog="tldr", usage="tldr command [options]", description="Python command line client for tldr"
    )
    parser.add_argument(
        "-v",
        "--version",
        action="version",
        version="%(prog)s {} (Client Specification {})".format(__version__, __client_specification__),
    )

    parser.add_argument("--search", metavar='"KEYWORDS"', type=str, help="Search for a specific command from a query")

    parser.add_argument(
        "-u", "--update", "--update_cache", action="store_true", help="Update the local cache of pages and exit"
    )

    parser.add_argument("-k", "--clear-cache", action="store_true", help="Delete the local cache of pages and exit")
    ...
    parser.add_argument(
        "-l", "--list", default=False, action="store_true", help="List all available commands for operating system"
    )

    parser.add_argument(
        "-s", "--source", default=PAGES_SOURCE_LOCATION, type=str, help="Override the default page source"
    )

    parser.add_argument(
        "-c", "--color", default=None, action="store_const", const=False, help="Override color stripping"
    )

    parser.add_argument("-r", "--render", default=False, action="store_true", help="Render local markdown files")

    parser.add_argument("-L", "--language", nargs=1, default=None, type=str, help="Override the default language")

    parser.add_argument("-m", "--markdown", default=False, action="store_true", help="Just print the plain page file.")

    parser.add_argument(
        "--short-options", default=False, action="store_true", help="Display shortform options over longform"
    )

    parser.add_argument(
        "--long-options", default=False, action="store_true", help="Display longform options over shortform"
    )

    parser.add_argument("command", type=str, nargs="*", help="command to lookup", metavar="command").complete = {
...
def main(options: Namespace) -> None:
    ...
    if options.list:
        ...
        for command in options.command:
            ...
            if file_path.exists():
                with file_path.open(encoding="utf-8") as open_file:
                    output(open_file.read().encode("utf-8").splitlines(), display_option_length, plain=options.markdown)
    elif options.search:
        ...
        try:
            ...
            results = get_page_for_every_platform(command, options.source, options.platform, options.language)
            if not results:
                sys.exit(
                    (
                ...
                if results[1:]:
                    ...
                    if are_multiple_platforms:
                        ...
                        print(f"Found 1 page with the same name under the platform: {platforms_str[0]}.")
        except URLError as e:
            sys.exit("Error fetching from tldr: {}".format(e))


def run_tldr(args: List[str]) -> str:
    """Run the embedded tldr CLI and capture its output."""
    f_out = StringIO()
    f_err = StringIO()
...
def lookup_command_examples(
    command: str,
    platform: Optional[str] = None,
    language: Optional[str] = None,
    search: bool = False,
) -> str:
    """Return plain tldr output for a command-oriented lookup."""
    normalized_command = command.strip()
    if not normalized_command:
        return "No command provided for tldr lookup."
    ...
    if search:
        args.extend(["--search", normalized_command])
    else:
        args.extend(shlex.split(normalized_command))

    return run_tldr(args)


class TLDRTool(Tool):
    """Integrated tldr lookup tool for command-specific grounding."""

    def get_name(self) -> str:
        return "lookup_tldr_command"
    ...
    async def execute(self, arguments: dict, config: HashConfig) -> str:
        command = str(arguments.get("command", "")).strip()
        platform = arguments.get("platform")
        language = arguments.get("language")
        search = bool(arguments.get("search", False))

        return lookup_command_examples(
            command=command,
